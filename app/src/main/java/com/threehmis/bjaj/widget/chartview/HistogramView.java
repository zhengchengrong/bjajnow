...
//		progress = new int[]{ 15, 5, 8, 12, 9 };// 5
//		ySteps = new String[] { "15", "12", "9", "6", "3", "0" };
		xWeeks = new String[] { "地基基础", "主体结构", "屋面", "装修", "竣工"};//设定初始值防止没数据
		text = new int[] { 0, 0, 0, 0, 0, 0, 0, 0};
		aniProgress = new int[] { 0, 0, 0, 0, 0 ,0, 0, 0};
...
//		bitmap = BitmapFactory.decodeStream(is);
	...
	public void start(int flag,double maxnum,int[] progress,String[] xWeeks) {
	...
	protected void onDraw(Canvas canvas) {
		...
		Log.d("CD",aniProgress.toString()+"=www");
		// 绘制矩形
		if (aniProgress != null && aniProgress.length > 0) {
			for (int i = 0; i < aniProgress.length; i++) {// 循环遍历将7条柱状图形画出来
				...
				paint.setColor(Color.parseColor("#3398DB"));// 字体颜色#6DCAEC
...
//
//					rect2.left = step * (i + 1)-dp2px(10);
//					rect2.right = dp2px(25) + step * (i + 1)+dp2px(10);
//					rect2.top =  dp2px(15);
//					rect2.bottom = height;
