...
//	private HistogramAnimation ani;
	...
	private void init(Context context, AttributeSet attrs) {
		...
		nums = new String[] {"100", "50", "0" };
		season = new String[] { "第一季度", "第二季度", "第三季度", "第四季度" };

		text = new int[] { 50, 100, 15, 10 };
	...
	protected void onDraw(Canvas canvas) {
		...
		int hPerHeight = leftHeight / 2;// 分成2部分
		...
		for (int i = 0; i < 4; i++) {// 画x线
			...
			x = i * step + dp2px(30);
			if (i == 0) {
				path.moveTo(x, (float) (leftHeight - text[i] * (leftHeight / 100.0) + dp2px(15)));
			} else {
				path.lineTo(x, (float) (leftHeight - text[i] * (leftHeight / 100.0) + dp2px(15)));
			}

			// 线上的点
			canvas.drawCircle(x, (float) (leftHeight - text[i] * (leftHeight / 100.0) + dp2px(15)), dp2px(4), linePaint);
		...
		// 点击后是否显示线性图上的标注
					if (this.text2[i] == TRUE) {
						...
						if (i == 3) {
							...
							rect.right = dp2px(75) + dp2px(10) + (i - 1) * step + dp2px(30);
							rect.top = height / 2 - dp2px(18);
							rect.bottom = height / 2 + dp2px(18);
							canvas.drawRect(rect, xLinePaint);

							// 标注字
							xLinePaint.setColor(Color.parseColor("#ffffff"));
							canvas.drawText(season[i] + "", dp2px(12) + (i - 1) * step + dp2px(30), height / 2 - dp2px(2),
									xLinePaint);

							canvas.drawText("不合格率:" + text[i], dp2px(24) + (i - 1) * step + dp2px(30), height / 2 + dp2px(12),
									xLinePaint);

							// 标注上的点
							xLinePaint.setColor(Color.parseColor("#bb2222"));
							canvas.drawCircle(dp2px(17) + (i - 1) * step + dp2px(30), height / 2 + dp2px(9), dp2px(3),
									xLinePaint);
						} else {
							// 通过画笔颜色话柱状标注
							xLinePaint.setColor(Color.parseColor("#99000000"));// 字体颜色
							...
							rect.right = dp2px(75) + dp2px(10) + i * step + dp2px(30);
							rect.top = height / 2 - dp2px(18);
							rect.bottom = height / 2 + dp2px(18);
							canvas.drawRect(rect, xLinePaint);
							...
							// 标注字
							xLinePaint.setColor(Color.parseColor("#ffffff"));
							canvas.drawText(season[i] + "", dp2px(12) + i * step + dp2px(30), height / 2 - dp2px(2),
									xLinePaint);

							canvas.drawText("不合格率:" + text[i], dp2px(24) + i * step + dp2px(30), height / 2 + dp2px(12),
									xLinePaint);

							// 标注上的点
							xLinePaint.setColor(Color.parseColor("#bb2222"));
							canvas.drawCircle(dp2px(17) + i * step + dp2px(30), height / 2 + dp2px(9), dp2px(3), xLinePaint);
						}
					}
		 }
