...
import static com.threehmis.bjaj.module.more.video.CommonUtils.SIZE_1;
import static com.threehmis.bjaj.module.more.video.CommonUtils.SIZE_2;
import static com.threehmis.bjaj.module.more.video.CommonUtils.getScreenHeight;
import static com.threehmis.bjaj.module.more.video.CommonUtils.getScreenWidth;
import static com.threehmis.bjaj.utils.ImageFactory.compressImage;
...
public class RecordActivity extends AppCompatActivity implements SurfaceHolder.Callback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        videoFlashLight.setOnClickListener(new clicklistener());
        swicthCamera.setOnClickListener(new clicklistener());
        recordButton.setOnClickListener(new clicklistener());
...
//        recordButton.setImageResource(R.drawable.stop_record);   //录制按钮变成待停止
        recordButton.setBackgroundResource(R.drawable.stop_record);   //录制按钮变成待停止
    }
...
//        recordButton.setImageResource(R.drawable.record);   //录制按钮变成待停止
        recordButton.setBackgroundResource(R.drawable.record);   //录制按钮变成待停止
    }
            ...
            if (startRecord()) {
                ...
                videoTime.setOnChronometerTickListener(new Chronometer.OnChronometerTickListener() {
                    ...
                    public void onChronometerTick(Chronometer chronometer) {
                        //最大录制时长 if (chronometer.getText().equals("00:61"))
...
//        startActivityForResult(intent, 2222);
    ...
    public String videoDir() {
        ...
        Log.d("CD", "videoFilelength==" + videoFile.getAbsolutePath());
        return null;
    }
    ...
    private void getInteract(File file) {


        Log.d("CD", "vvvvvvvv=" + file.length());

        File videoimageFile = saveBitmapFile(getBitmapFormUrl(file.getAbsolutePath()));

        Log.d("CD", "videoimageFile=" + videoimageFile.length());
        ...
        dialog.show();
          ...
          if (isActionRegist)//是行为登记和实体登记视频
              AndroidApplication.getInstance().doPostAsyncvideo(RetrofitFactory.BASE_URL + "attachment/saveVideo.action", file,  videoimageFile, "checkId", id, "video", new okhttpcall());
        else {
              if (isquestion)
                  AndroidApplication.getInstance().doPostAsyncvideo(RetrofitFactory.BASE_URL + "attachment/saveVideo.action", file, videoimageFile, "questionId", id, "video", new okhttpcall());
              else
                  AndroidApplication.getInstance().doPostAsyncvideo(RetrofitFactory.BASE_URL + "attachment/saveVideo.action", file, videoimageFile, "checkId", id, "video", new okhttpcall());
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case RetrofitFactory.MSG_SUCESS:
                    ...
                    Toast.makeText(RecordActivity.this, "上传视频成功!", Toast.LENGTH_SHORT).show();
                    finish();

                    //删除本地视频文件
                    FileUtils.deleteFile(videoFile.getAbsolutePath());
                ...
                case RetrofitFactory.MSG_FAIL:
                    ...
                    dialog.cancel();
                    Toast.makeText(RecordActivity.this, "上传视频失败!", Toast.LENGTH_SHORT).show();
                    finish();
                    //删除本地视频文件
                    FileUtils.deleteFile(videoFile.getAbsolutePath());
     ...
     * @return
     */
    private Bitmap getBitmapFormUrl(String path) {
        Bitmap bitmap = null;
    ...
    private File saveBitmapFile(Bitmap bitmap) {
        ...
        Bitmap bm = compressImage(bitmap);//质量压缩方法返回bitmap

        File file = new File("mnt/sdcard/DCIM/Camera/loading.jpg");//将要保存图片的路径
        try {
